...
// var ringbufferjs = require('ringbufferjs');
...
function run(callback, tearDownCallback) {
...
function spawnUsingPidLock(config, callback) {
    ...
    pidlock.guard(lockdir, config.storer_lockname, function(error, data, cleanup) {
        if (error) {
            ...
            run(callback, function(tearDownCallback) {
...
function spawnUsingFaye(config, callback) {
    var app = express();
    app.get('/', function(request, response) {
        response.send('OK');
    });

    console.log('Faye: Subscribing to channel /' + config.pubsub_channel + ' on port ' + config.pubsub_port + '.');
    var bayeux = new faye.NodeAdapter({ mount: '/' + config.pubsub_channel });

    var server = http.createServer(app);
    bayeux.attach(server);
    server.listen(config.pubsub_port, function() {
...
module.exports.spawn = function(config, callback) {
    ...
    var spawn = (config.pubsub_channel && config.pubsub_port) ? spawnUsingFaye : spawnUsingPidLock;
