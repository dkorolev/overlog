...
Impl.prototype.ensureFileIsOpen = function() {
    if (_.isUndefined(this.fd)) {
        ...
        this.file_start_time_ms = Date.now();
...
Impl.prototype.stats = function(unittest_mode) {
    var result = {
        ...
        current_file: {
          number_of_entries: this.entries,
          time_interval: this.ms
        },
    };
    if (!unittest_mode) {
        result.current_file.fn = this.fn;
        result.uptime_in_seconds = 1e-3 * (Date.now() - this.start_time_ms);
        result.current_file.age_in_seconds = 1e-3 * (Date.now() - this.file_start_time_ms);
        ...
        if (msq_size > 0) {
            result.qps_on_recent_entries = msq_size / (1e-3 * (Date.now() - msq.peek_least_recent()));
        }
    }
    return result;
};
...
function runStorer(userCodeCallback, tearDownCallback, extraCallbacks) {
...
function spawnUsingPidLock(config, userCodeCallback) {
    ...
    pidlock.guard(lockdir, config.storer_lockname, function(error, data, cleanup) {
        if (error) {
            ...
            runStorer(userCodeCallback, function(tearDownCallback) {
...
function spawnUsingFaye(config, userCodeCallback) {
    ...
    var statsCallback = function() { return { status: 'OK' } };
    ...
    server.listen(pubsub_port, function() {
        ...
        runStorer(userCodeCallback, function() {
