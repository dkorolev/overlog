...
module.exports.spawn = function(config, callback) {
    ...
    Fetcher.prototype.follow = function(callback) {
        var self = this;
        assert(_.isFunction(callback));
        var server_path = self.params.pubsub_server + ':' + self.params.pubsub_port + self.params.pubsub_mount;
        this.log('PubSub: connecting to "' + server_path + '".');
        var client = new faye.Client(server_path);
        client.subscribe('/log', function(message) {
            if (_.isObject(message) && _.isObject(message.entry)) {
                callback(message.entry);
            } else {
                this.log('Ignoring malformed PubSub message: ' + JSON.stringify(message));
            }
        });
    };
...
if (require.main === module) {
    ...
    module.exports.spawn(config, function(fetcher) {
        ...
        if (config.since_ms_then_follow) {
            fetcher.fetch({ begin_ms: config.since_ms_then_follow }, dump);
            follow = true;
        }
        if (config.last_ms_then_follow) {
            fetcher.fetch({ begin_ms: Date.now() - config.last_ms_then_follow }, dump);
            follow = true;
        }
        if (follow) {
