...
module.exports.spawn = function(config, callback) {
        ...
        if (Object.hasOwnProperty.call(this.files, fn)) {
            ...
            if (!fs.existsSync(full_fn)) {
                ...
                if (fn.substr(-4) !== '.log' ||
                    ...
                    this.sorted_files.sort(function(a, b) { return b.ms.last - a.ms.last; });
                }
            }
        }
    };
    ...
    callback({
        fetch: function(q, callback) {
            ...
            var query = q || { begin_ms: null, end_ms: null };
                ...
                step(
                    function() {
                        ...
                        for (var i = 0; i < fetcher.sorted_files.length; ++i) {
                            ...
                            if ((!query.begin_ms || query.begin_ms <= f.ms.last) &&
                                (!query.end_ms || query.end_ms >= f.ms.first) ) {
                                var cb = group();
                                f.read({
                                    entry: function(e) {
                                        if (_.isObject(e) && Object.prototype.hasOwnProperty.call(e, 'ms')) {
                                            if ((!query.begin_ms || e.ms >= query.begin_ms) &&
                                                (!query.end_ms || e.ms < query.end_ms)) {
                                                result.push(e);
                                            }
                                        }
                                    },
        ...
        rl.on('line', function(untrimmed_line) {
            ...
            if (line === '' || line === 'ALL') {
                ...
                if (q.length === 1) {
                    var a = Number(q[0]);
                    if (a) {
                        fetcher.fetch({ begin_ms: a }, callback);
                    } else {
                        synopsis();
                    }
                } else if (q.length === 2) {
                    var a = Number(q[0]), b = Number(q[1]);
                    if (a && b) {
                        fetcher.fetch({
